...
utils.plot_model(model, to_file="modified_LeNet.png")
...
# 모델을 학습합니다. (shuffle을 하지 않습니다.)
results_no_shuffle = model.fit(train_images, train_labels, epochs=10, shuffle=False)
no_shuffle_test_loss, no_shuffle_test_acc = model.evaluate(test_images, test_labels)

# 모델을 학습합니다. (shuffle을 사용해 봅니다.)
results_shuffle = model.fit(train_images, train_labels, epochs=10, shuffle=True)
shuffle_test_loss, shuffle_test_acc = model.evaluate(test_images, test_labels)

# epoch별 loss를 비교 해 봅니다.
